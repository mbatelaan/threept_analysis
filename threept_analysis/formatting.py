...
def err_brackets(quantity, error, error_prec=2, form='Mix', texify=False, decimal_in_error=False):
    ...
    check_q =( ( len(f'{quantity:#.{quantity_prec:}g}'.split('e')))>1
               if quantity_prec>0 else False)
    ...
    if form=='Mix' and not is_exp_required:
        ...
        #normal
            ...
            if len(error_temp_str)>error_prec: rounded=True
        ...
        if exp_diff>=0 and quantity_prec>0:
            ...
            if quantity<0:
                quantity_string = '-'+'0'*(-exp_diff)+f'{abs(quantity):#.{quantity_prec}g}'
            else:
    ...
    if form=='Sci' or is_exp_required:
        ...
        if decimal_in_error==True and (error_exp_shift-error_prec+1)<=0:
            ...
            error_temp = round(error_sig*10**(error_prec-1))*10**(error_exp_shift-error_prec+1)
            error_string=f'({error_temp:})'
        else:
            error_temp_str = str(round(error_sig *10**(error_prec-1)))
            #if error rounds up an order, drop a digit and flag
        ...
        #significand
        #normal
        if exp_diff>=0 or (exp_diff<0 and decimal_in_error==True and quantity_prec>0):
        ...
        # need to add leading zeros AFTER negative signs
            ...
            if quantity_sig<0:
                quantity_string = '-'+'0'*(-exp_diff)+f'{abs(quantity_sig):#.{quantity_prec-1}f}'
            else:
